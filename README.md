...
- 通过蓝牙 BLE 读取和写入频率数据，蓝牙读写频使用官方 APK 同款 `0x80` 原生块链路。
...
8800Pro 的 FFE0/FFE1 蓝牙链路可以完成握手、读频和写频。调试过程中曾经出现过 `404.00657`、`412.00757` 这类乱码频率，核心原因是浏览器端曾按 USB 写频线的 `0x40` 块格式组织蓝牙写入，导致机器把边界解释错，正常信道被异常数据覆盖。

当前实现已按官方 Android APK 反编译确认的蓝牙原生链路重做：

- 握手和普通读写块都走 `FFE1`。
- 蓝牙读取使用 `52 addr 80`，一次读取 128 字节。
- 蓝牙写入使用 `57 addr 80 + 128 bytes`，网页内部再拆分/合并为 64 字节内存块。
- 只有在已有完整读频快照、且只改了少量信道时，才启用快速写入跳过无变化块；其他情况走全量官方块。
